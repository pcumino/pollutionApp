...
int main(int argc, char* argv[]) {
    ...
    try {
        // Create a buffer where all objects will live. Use a sensible initial buffer size and set the buffer to automatically grow if needed.
        const size_t initial_buffer_size = 10000;
        osmium::memory::Buffer buffer{initial_buffer_size, osmium::memory::Buffer::auto_grow::yes};

        // Add nodes to the buffer. This is, of course, only an example. You can set any of the attributes and more tags, etc. Ways and relations can be added in a similar way.
        ...
        // Initialize Writer using the header from above and tell it that it is allowed to overwrite a possibly existing file.
        osmium::io::Writer writer{output_file_name, header, osmium::io::overwrite::allow};

        // Write out the contents of the output buffer.
        writer(std::move(buffer));

        // Explicitly close the writer. Will throw an exception if there is a problem. If you wait for the destructor to close the writer, you will not notice the problem, because destructors must not throw.
        writer.close();
